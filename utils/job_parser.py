...
def parse_job(krama_root=None, job_config_path=None):
    """
    ...
    Args:
        krama_root (string): Path or expresso root directory, mostly it is '/usr/share/expresso'
...
def get_category_path(arg=None, type_val='data',
    ...
    .. code-block:
        $krama_root/storage/job/computer_vision/dataset/_to_hdf5/mnist_dataset
...
def merge_with_parent_arg(arg=None, arg_type='data', krama_root=None):
    """
    ...
    Args:
        ...
        krama_root: Path of expresso's installation location. Mostly it is '/usr/share/expresso'
    ...
    parent_arg_path = get_category_path(arg=arg, type_val=arg_type,
                                        krama_root=krama_root)

    if (parent_arg_path is None): return arg;
    parent_config_arg = krama_pb2.Arg()
    text_format.Merge(
...
def merge_with_parent_job(job=None, krama_root=None):
    """
    ...
    if parent_job_path is not None:
        ...
        parent_config_job = krama_pb2.Job()
    ...
    for idx, data_arg in enumerate(job.config):
        arg_val = merge_with_parent_arg(arg=job.config[idx],
                                        arg_type='config', krama_root=None);
        if arg_val is None: continue
        job.config[idx].CopyFrom(arg_val)

    return job

def get_job(krama_root,job_config_path):
    """
    ...
    """
    job=parse_job(krama_root=krama_root,job_config_path=job_config_path)
    job=make_job_paths_absolute(job=job,job_config_path=job_config_path)
    job=merge_with_parent_job(job=job,krama_root=krama_root)
    return job;
